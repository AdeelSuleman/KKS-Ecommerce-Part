...
const CartPage = () => {
  ...
  // Optimization: Grouping logic with memoization
  ...
  // Optimization: Price calculation memoization
  const subtotal = useMemo(() => {
    return uniqueItems.reduce((sum, item) => sum + (item.price || 0) * item.qty, 0);
  }, [uniqueItems]);
  ...
  const increase = useCallback((item) => {
    addToCart({
      ...item,
      _cartItemId: `${item.id}-${item.selectedWeight}-${item.packaging}-${Date.now()}`
    });
  }, [addToCart]);

  const decrease = useCallback((item) => {
    const target = cart.find(
      (i) => `${i.id}-${i.selectedWeight}-${i.packaging}` === item._groupKey
    );
  ...
  useEffect(() => {
    document.body.style.overflow = (showForm || showThankYou) ? "hidden" : "auto";
    return () => { document.body.style.overflow = "auto"; };
  }, [showForm, showThankYou]);

  return (
    <main
      // Design & UI kept exactly as original
    ...
    >
      <div className="xs:w-[95vw] xl:w-7xl px-4 lg:px-6 z-20 relative">
        <div className="grid lg:grid-cols-2 justify-center xs:gap-8 lg:gap-6 xl:gap-20 ">
          ...
          <div className=" bg-black/20 backdrop-blur-xs rounded-xl border border-textGray p-6">
            <h2 className="font-Poppins font-bold text-textSecondary xs:text-Heading5 xs:text-left md:text-Heading4 lg:text-Heading4 xl:text-Heading3">
              Your Shopping Cart
            </h2>
              ...
              >
                <div className="flex gap-3">
                  <img
                    src={item.p_image}
                    alt={item.p_name}
                    loading="eager" // Performance fix for LCP
          ...
          <div className="bg-black/10 backdrop-blur-sm rounded-xl border border-textGray p-6 h-fit">
            ...
            <div className="lg:flex lg:gap-3 space-y-5 lg:space-y-0 mt-6">
              ...
              {/* <Link to="/"> */}
              <Link to="/" className="bg-btnPrimary flex flex-col items-center-safe text-white font-semibold xs:px-4 xs:py-3 lg:px-3 lg:py-2 xl:px-4 xl:py-3 lg:text-Paragraph7 xl:text-Paragraph5 cursor-pointer rounded-lg w-full focus:ring-0 focus:outline-none hover:bg-btnPrimaryHover">
                Continue Shopping
              </Link>
              {/* </Link> */}
            </div>
      ...
      <div className="absolute w-full top-0 z-0">
        <img src={Banner} alt="" fetchPriority="low" className="object-cover w-full h-full" />
      </div>
      ...
      { showForm && (
        <CheckoutForm 
          ...
          onSuccess={() => { setShowForm(false); setShowThankYou(true); }}
        />
      )}
