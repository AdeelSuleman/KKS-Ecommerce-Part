export const Menus = [
    { nav_Title: "Home", nav_Url: "/" },
    // { nav_Title: "About", nav_Url: "/about" },
  
    {
      nav_Title: "Our Products",
      nav_Url: "/shop",
      sub_Menu: [
        { nav_SubTitle: "Recipe Mixes", nav_SubUrl: "/recipe-mixes" },
        { nav_SubTitle: "Plain Spices", nav_SubUrl: "/plain-spices" },
        { nav_SubTitle: "Desserts", nav_SubUrl: "/Desserts" },
        { nav_SubTitle: "Pickles", nav_SubUrl: "/Pickles" },
        { nav_SubTitle: "Sauces", nav_SubUrl: "/Sauces" },
        { nav_SubTitle: "Pastes", nav_SubUrl: "/Pastes" },
        { nav_SubTitle: "Miscellaneous", nav_SubUrl: "/Miscellaneous" },
      ],
    },
  
    { nav_Title: "Shop", nav_Url: "/shop" },
    { nav_Title: "Contact Us", nav_Url: "https://wa.me/+923369917755" },
  ];